...
def save_midlayer_state(path_config: PathConfig, layer_idx: int, batch_idx: int, state: torch.Tensor):
    """Save intermediate layer output to a file in FP8 format"""
    save_quant(state, path_config.get_midlayer_path(layer_idx, batch_idx))


def load_intermediate_state(path_config: PathConfig, layer_idx: int, batch_idx: int, batch_size) -> torch.Tensor:
...
def load_midlayer_state(path_config: PathConfig, layer_idx: int, batch_idx: int, batch_size) -> torch.Tensor:
    """Load intermediate layer output from a file and upcast from FP8"""
