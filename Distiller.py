...
class OptimizerModeCallback(Callback):
    def on_train_epoch_start(self, trainer, pl_module):
        # Call .train() on the optimizer
        optimizer = trainer.optimizers[0]
        if hasattr(optimizer, "train"):
            optimizer.train()
        print("Optimizer set to train mode.")

    def on_validation_epoch_start(self, trainer, pl_module):
        # Call .eval() on the optimizer
        optimizer = trainer.optimizers[0]
        if hasattr(optimizer, "eval"):
            optimizer.eval()
        print("Optimizer set to eval mode.")
...
def prepare_distilled_moe(
    ...
    AdapterClass = (
        DoRALinear if distillation_config.lora_type.lower() == "dora" else LoRALinear
...
class MOEDistillerLightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=(self.params.n_epochs * self.params.n_train_batch) // self.params.gradient_accumulation_steps,
            eta_min=self.params.learning_rate * 0.1,  # Minimum learning rate
        )

        warmup_scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.1, total_iters=64)

        scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[warmup_scheduler, scheduler], milestones=[64])
