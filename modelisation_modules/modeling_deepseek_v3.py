...
class DeepseekV3YarnRotaryEmbedding(DeepseekV3RotaryEmbedding):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        _mscale = float(
            yarn_get_mscale(self.scaling_factor, self.mscale) / yarn_get_mscale(self.scaling_factor, self.mscale_all_dim)
        )
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            if self.seq_aux:
                ...
                aux_loss = (Pi * fi).sum() * 0.001
        else:
            aux_loss = None

        if self.save_path is not None:
            with open(self.save_path, "wb") as f:
                pickle.dump(np.array(topk_idx.detach().cpu().numpy()), f)
...
class DeepseekV3MoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "ep_size") and config.ep_size > 1:
            ...
            self.ep_rank = 0
            self.experts = nn.ModuleList(
                [DeepseekV3MLP(config, intermediate_size=config.moe_intermediate_size) for i in range(config.n_routed_experts)]
            )
